...
config = PluginConfig(
    plugin=CoreConfig(
        reference='businesswire',         # уникальное имя источника
        type=SOURCE,                            # Тип источника (SOURCE, ML, PIPELINE)
        files=['businesswire.py', ],        # Список файлов, которые будут использоваться в плагине (эти файлы будут сохраняться в платформе)
    ...
    payload=payload.PayloadConfig(
        file='businesswire.py',                 # python файл плагина (точка входа). Этот файл должен быть указан в `plugin.files[*]`
        classname='BUSINESSWIRE',               # имя python класса в указанном файле
